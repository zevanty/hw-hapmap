 ...
 * searched one at a time.  This class performs an insertion check on the reference
 * string, meaning if search string was 'CAT' and reference string was 'CAAT', it will
 * still be marked as valid find.  This will not handle a mutation.
 */

import java.io.BufferedReader;
import java.io.FileReader;

public class SequenceFinderSimpleInsertion {
	
	//performs the search
	public long countSubstr(String substr, String sequence) throws Exception {
		...
		System.out.println("Position\tNumInsertions");		
			...
			case 't':
				...
				if (tempSeqPartition.length() >= maxStrLen) {
					...
					for (i = 0; i < maxStrLen; i++) {
						//record 1st position of comparision
						if(currPos == 0) {
							startPos = i;
						}
						...
						//if mismatch
							...
							//if not reach error threshold yet, ignore error (insertion)
